...
public class CollectComponent : MonoBehaviour
{
    ...
    public bool PetToCollect(Rigidbody _rb)
    {
        ...
        RaycastHit hit;
        if(Physics.Raycast(ray, out hit, .7f) && hit.transform.tag == "Pet" && _petId != hit.transform.GetComponent<PetController>().GetSerialId){
            _petGrabEvent.GrabPet(hit.transform.GetComponent<PetController>().GetSerialId);
            _petId = hit.transform.GetComponent<PetController>().GetSerialId;
